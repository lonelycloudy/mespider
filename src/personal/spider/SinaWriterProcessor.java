...
*/
public class SinaWriterProcessor
extends Processor implements CoreAttributeConstants {
    ...
    */
    ...
    private static final Logger logger =
        Logger.getLogger(SinaWriterProcessor.class.getName());

    /**
     * @param name Name of this processor.
     */
    public SinaWriterProcessor(String name) {
    ...
    /**
     * 自定义的innerProcess方法,抓取的内容提取属性并入库.
     * 来源SinaExtractor
     * 20130819 lx
     * */
    protected void innerProcess(CrawlURI curi){
		...
		try{ 
			...
			hr.setCharacterEncoding("gb2312");//gb2312
		...
		try{
			...
			if(url.contains("http://tech.sina.com.cn/t/")){
				...
				for(Element article : articles){
					...
					String source  = eSource.child(0).text();
					//source = new String(source.getBytes());
					String sourceUrl  = eSource.child(0).attr("href");
					...
					System.out.println("FI:"+"title"+title+"pubdate"+pubdate+"source"+source+"sourceUrl"+sourceUrl);
				}
			}else{
				...
				for(Element row : mainrow){
					...
					for (Element link : links) {
					  ...
					  System.out.println("FLL:"+linkHref+"FLT:"+linkText);
					}
				}
			}
		}catch(Exception e){
    ...
    protected void innerProcessBak(CrawlURI curi) {
        if (!curi.isSuccess()) {
            return;
        }
    ...
    */
    private void writeToPath(RecordingInputStream recis, File dest)
        throws IOException {
    	//20130819 commit bellow
    	//System.out.println("F:"+recis);
        /*   
